...
$apiCmd = "Set-Location '$ApiDir'; `$env:MYSQL_DATABASE_URL='$($env:MYSQL_DATABASE_URL)'; `$env:DATABASE_BACKEND='$($env:DATABASE_BACKEND)'; `$env:CORS_ORIGINS='$($env:CORS_ORIGINS)'; `$env:APP_PUBLIC_URL='$($env:APP_PUBLIC_URL)'; & '$Python' -m uvicorn app.main:app --reload --host 127.0.0.1 --port $ApiPort"
